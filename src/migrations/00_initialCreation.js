...
const up = async ({ context: queryInterface, dialect }) => {
  // state
  tables.push({
    tableName: 'states',
    fields: {
  ...
  tables.push({
    ...
    fields: {
      ...
      roles: {
        type: dialect === 'postgres' ? { type: Sequelize.ARRAY(Sequelize.TEXT) } : Sequelize.TEXT
      },
      targetLinkUri: {
        type: Sequelize.TEXT
      },
  ...
  tables.forEach(async (tableConfig) => {
    await queryInterface.createTable(tableConfig.tableName, { ...tableConfig.fields, ...timeDefaults })
